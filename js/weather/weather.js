	...
	$.ajax({
		...
		success: function (data) {
			...
			$(this.windSunLocation).updateWithText(_newWindHtml + ' ' + _newSunHtml,this.fadeInterval);
                        $(this.cityLocation).updateWithText(this.params.q);
...
weather.init = function () {
        ...
        if (this.index > 0) {
            this.cityLocation = this.cityLocation + this.index;
            this.temperatureLocation = this.temperatureLocation + this.index;
            this.windSunLocation = this.windSunLocation + this.index;
            this.forecastLocation = this.forecastLocation + this.index;
            this.params = weather.params[this.index];
        }
